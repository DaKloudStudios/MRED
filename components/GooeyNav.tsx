...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  particleR = 40,
  timeVariance = 200,
  colors = [1, 2, 1, 3], // Refers to CSS vars --color-1, etc.
  currentId,
  onItemClick,
  className = ''
}) => {
  ...
  // Sync active state with props
  useEffect(() => {
    if (currentId) {
        const idx = items.findIndex(i => i.id === currentId);
        // We only set active index if it's found. 
        // If currentId is 'contact' (not in these items), activeIndex will be -1 (no pill).
        setActiveIndex(idx);
    }
  }, [currentId, items]);
  ...
  const updateEffectPosition = (element: HTMLElement) => {
    ...
    // Update text content to match the active item
    const link = element.querySelector('a');
    if (link) textRef.current.innerText = link.innerText;
  };
  ...
  // React to resize or active index change to reposition the blob
  useEffect(() => {
    if (!navRef.current || !containerRef.current) return;
    
    const update = () => {
        if (activeIndex === -1) {
            // Hide if no active item
